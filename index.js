...
class Scraper {
  ...
  async getMangaSearch(mangaSearch) {
    ...
    let pageNum = 1;
    let pages = [];
    await axios.get("https://manganelo.com/search/story/" + mangaSearch + "?page=" + pageNum)
      .then(async res => {
  ...
  async getSearchURL(mangaName) {
    let urls = [];
    return (
      await this.getMangaSearch(mangaName)
        .then(async (res) => {
          for(let page of res) {
  ...
  getMangaDataFromSearch(mangaName) {
    return (
      this.getSearchURL(mangaName)
        .then(async links => {
          let mangas = [];
          for(let link of links) {
            let messyData = [];
            let name = null;
            await this.getMangaPage(link)
              .then(res => {
                const $ = cheerio.load(res.data);
                ...
                let tempOne = [];
                $('.variations-tableInfo').children().first().children().each((i, elm) => {
                ...
                let tempTwo = [];
                $('.story-info-right-extent').children().each((i, elm) => {
                ...
                let chapters = [];
                $('.row-content-chapter').children().each((i, elm) => {
